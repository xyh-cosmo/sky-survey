...
//      ========================================================================
        ...
        if( p_rank == 0 && sky_id_tracker_cnt2 >= 10 ){
            double yr = (curTime-2459766.0)/365.25;
            printf("> t = %6.4f yr,",yr);
            printf(" %8d patches to be observed,", sky_num_remained);
            printf(" d-area = %12.6f,",area1);
            printf(" ud-area = %12.6f\n",area2);
            sky_id_tracker_cnt2 = 0;
        }
    ...
    if ( p_rank == 0 ) {
        fprintf(outf, "#执行的次数为：  %d    未执行的次数为:    %d   \n", excuteNum, noExcuteNum);
        fprintf(outf, "# sun angle with orbit > 15 time is %f\n", toearthTime);
