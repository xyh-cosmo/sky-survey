...
double isInDeepSurveyArea(double ra, double dec, double diam) {
    ...
    // double areaCenter[10][2] = { { 266.85, -5.59 },
    //                              { 77.50, 6.09 },
    ...
    double areaCenter[10][2] = { {266.85,  -5.59},  // galactic center
    ...
    if(transAngle < 1) {
        ...
        for(i = 0; i < 3 ; i ++) {
            if(transAngle>data[i][0] && transAngle <= data[i + 1][0] ) {
                tTime = data[i][1] * ((transAngle - data[i+1][0])) / (((data[i][0] - data[i+1][0]))) + data[i+1][1] * ((transAngle - data[i][0])) / (((data[i+1][0] - data[i][0])));
                break;
            }
        }
    }
