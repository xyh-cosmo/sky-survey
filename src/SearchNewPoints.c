...
//  计算sky_id_tracker[]的指标范围
    ...
    if ( isInSunSide == 1 ) {// 在阳照区时
        ...
        // double thres = (double) all_sky_num / p_size;
        // double thres = (double) sky_num_remained / p_size;

        // for (i = 0; i < thres; i++) {
        for( i=idx_start; i<idx_end; i++ ){

            // int s_id = i * p_size + p_rank;
            // int s_id = sky_id_tracker[i * p_size + p_rank];
            // int s_id = i;
            int s_id = sky_id_tracker[i];

            // s_id不可能超过所划分的天区的总的数目，因此如果出现s_id>=all_sky_num，就直接结束循环（因为s_id是由小变大的）
            // if (s_id >= all_sky_num) {
            //     break;
            // }
...
//              根据太阳和帆板法线的夹角来修正权重，尽量观测小角度的天区
                ...
                if (skyMap[s_id].inDeepFlag < 0) {
                    ...
                    if ( fabs(skyMap[s_id].gb) <= Galaxy_B_Sec_Low )
                        weight += MAX_VALUE/3.0;

                    if ( fabs(skyMap[s_id].dec) <= Ecliptic_Lat_Sec_Low )
                        weight += MAX_VALUE/4.0;
...
/************************************************************************/
        ...
        // double thres = (double) (all_sky_num) / (p_size - 1);
        // double thres = (double) sky_num_remained / p_size;

        int temp_i = 0;
        // for ( temp_i = 0; temp_i < thres; temp_i++ ) {
        for( temp_i=idx_start; temp_i<idx_end; temp_i++ ){

            // i = temp_i * p_size + p_rank;
            // i = sky_id_tracker[temp_i * p_size + p_rank];
            // i = temp_i;
        ...
        //  CMG
            ...
            CoordinateSpinEquatorial2Ecliptic(ttSatE, tSatE);
            double unSPoint[2];
            calculateSatEarthPoint(greenStartTime, initalGreenWithRa, curTime + (tTime + exTime)/86400.0, ttSatE, unSPoint, infp);
        ...
        ////////////////////////////////////////////////////////////////////
            ...
            if ( skyMap[i].inDeepFlag < 0 ) {
                ...
                if ( fabs(skyMap[i].gb) < Galaxy_B_Sec_Low ) {
                    weight += MAX_VALUE/3.0;
                    //                      skyMap[xLen * node.yPos + node.xPos].weight = 1;
                }
                if ( fabs(skyMap[i].dec) <= Ecliptic_Lat_Sec_Low ) {
                    weight += MAX_VALUE/4.0;
                    //                      skyMap[xLen * node.yPos + node.xPos].weight = 1;
                }
